...
import { PlusCircle, List, ClipboardCheck, Menu, X, LogOut, Bell } from "lucide-react";
...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { to: "/admin/users", label: "إدارة المستخدمين", Icon: List },
    { to: "/admin/notification", label: "الاشعارات", Icon: Bell },
  ];

  const handleLogout = async () => {
    try {
      Cookies.remove("adminToken", { path: "/", secure: true, sameSite: "lax" });
  ...
  return (
    <>
      <button
        ...
        className="fixed top-4 left-4 z-50 bg-teal-600 p-3 rounded-xl text-white 
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-linear-to-b
           from-teal-900 via-teal-800 to-amber-900 text-white
      ...
      >
        <div className="flex flex-col h-full justify-between py-10 px-6">
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-center mb-6">لوحة الإدارة</h2>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-xl transition-all
                                ${
                                  isActive
                                    ? "bg-linear-to-r from-amber-400 to-blue-400 text-white shadow-lg scale-105"
          ...
          <button
            ...
            className="flex items-center gap-2 justify-center mt-6 w-full px-4 py-3 
